...
class acc_ledger_ctrl extends base_ctrl {
	function __construct() {
		parent::__construct();		
	    $this->load->model('acc_ledger_model','model');
	}
	public function index()
	{
		if($this->is_authentic($this->auth->RoleId, $this->user->UserId, 'acc_ledger')){
			...
			$this->load->view('acc_ledger_view', $data);
		}
		else
		{
	...
	public function get_acc_group_list(){
		print  json_encode($this->model->get_acc_group_list());
	}
