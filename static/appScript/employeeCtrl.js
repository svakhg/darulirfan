...
function employeeCtrl($scope, $http){	
	...
	loadGridData($scope.pagingOptions.pageSize,1);
			loadData('get_designation_list',{}).success(function(data){$scope.designationList=data;});
	...
	$scope.saveItem=function(){	
		...
		angular.extend(record,$scope.item);
				record.designation_Designation_Name=undefined;
	...
	function loadData(action,data){
		return $http({
			  method: 'POST',
			  url: BASE_URL+'employee_ctrl/'+action,
			  data: data,
 ...
 employeeCtrl.prototype.init=function($scope){
	$scope.search=null;
	$scope.item=null;
	$scope.list = null;
...
employeeCtrl.prototype.configureGrid=function($scope){
	$scope.totalItems = 0;
    $scope.pagingOptions = {
    ...
    $scope.gridOptions = { 
        ...
        columnDefs: [
				...
				,{field: 'father_name', displayName: 'father_name'}
				,{field: 'mother_name', displayName: 'mother_name'}
				,{field: 'designation_Designation_Name', displayName: 'designation'}
				,{field: 'contact_no', displayName: 'contact_no'}
...
employeeCtrl.prototype.searchPopup=function($scope){
