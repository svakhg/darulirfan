...
function acc_group_typeCtrl($scope, $http) {
    $scope.auth = getAuth();
    this.init($scope);
    ...
    function isSearch() {
        ...
        for (var prop in $scope.search) {
            if ($scope.search.hasOwnProperty(prop) && $scope.search[prop])
                return true;
        }
        return false;
    }
...
acc_group_typeCtrl.prototype.init = function ($scope) {
    $scope.search = null;
    $scope.item = null;
    $scope.list = null;
...
acc_group_typeCtrl.prototype.configureGrid = function ($scope) {
    $scope.totalItems = 0;
    $scope.pagingOptions = {
...
acc_group_typeCtrl.prototype.searchPopup = function ($scope) {
