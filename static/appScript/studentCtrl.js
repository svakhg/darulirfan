...
function studentCtrl($scope, $http){	
	...
	loadGridData($scope.pagingOptions.pageSize,1);
		...
		loadData('get_class_list',{}).success(function(data){$scope.classList=data;});
	...
	$scope.saveItem=function(){	
		...
		record.class_class_name=undefined;
	...
	$scope.editItem=function(row){	
		...
		$scope.item.roll_no=Number($scope.item.roll_no);
	...
	function loadData(action,data){
		return $http({
			  method: 'POST',
			  url: BASE_URL+'student_ctrl/'+action,
			  data: data,
 ...
 studentCtrl.prototype.init=function($scope){
	$scope.search=null;
	$scope.item=null;
	$scope.list = null;
...
studentCtrl.prototype.configureGrid=function($scope){
	$scope.totalItems = 0;
    $scope.pagingOptions = {
    ...
    $scope.gridOptions = { 
        ...
        columnDefs: [
				...
				,{field: 'std_name', displayName: 'std_name'}
...
studentCtrl.prototype.searchPopup=function($scope){
