...
function std_feesCtrl($scope, $http){	
	$scope.auth=getAuth();
	this.init($scope);	
	...
	function loadData(action,data){
		return $http({
			  method: 'POST',
			  url: BASE_URL+'std_fees_ctrl/'+action,
			  data: data,
 ...
 std_feesCtrl.prototype.init=function($scope){
	$scope.search=null;
	$scope.item=null;
	$scope.list = null;
...
std_feesCtrl.prototype.configureGrid=function($scope){
	$scope.totalItems = 0;
    $scope.pagingOptions = {
...
std_feesCtrl.prototype.searchPopup=function($scope){
