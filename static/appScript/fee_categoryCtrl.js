...
function fee_categoryCtrl($scope, $http){	
	$scope.auth=getAuth();
	this.init($scope);	
	...
	function loadData(action,data){
		return $http({
			  method: 'POST',
			  url: BASE_URL+'fee_category_ctrl/'+action,
			  data: data,
 ...
 fee_categoryCtrl.prototype.init=function($scope){
	$scope.search=null;
	$scope.item=null;
	$scope.list = null;
...
fee_categoryCtrl.prototype.configureGrid=function($scope){
	$scope.totalItems = 0;
    $scope.pagingOptions = {
...
fee_categoryCtrl.prototype.searchPopup=function($scope){
