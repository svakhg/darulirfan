...
function statusCtrl($scope, $http){	
	...
	function loadData(action,data){
		return $http({
			  method: 'POST',
			  url: BASE_URL+'status_ctrl/'+action,
			  data: data,
 ...
 statusCtrl.prototype.init=function($scope){
	$scope.search=null;
	$scope.item=null;
	$scope.list = null;
...
statusCtrl.prototype.configureGrid=function($scope){
	$scope.totalItems = 0;
    $scope.pagingOptions = {
...
statusCtrl.prototype.searchPopup=function($scope){
