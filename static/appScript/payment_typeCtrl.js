...
function payment_typeCtrl($scope, $http){	
	$scope.auth=getAuth();
	this.init($scope);	
	...
	function loadData(action,data){
		return $http({
			  method: 'POST',
			  url: BASE_URL+'payment_type_ctrl/'+action,
			  data: data,
 ...
 payment_typeCtrl.prototype.init=function($scope){
	$scope.search=null;
	$scope.item=null;
	$scope.list = null;
...
payment_typeCtrl.prototype.configureGrid=function($scope){
	$scope.totalItems = 0;
    $scope.pagingOptions = {
...
payment_typeCtrl.prototype.searchPopup=function($scope){
