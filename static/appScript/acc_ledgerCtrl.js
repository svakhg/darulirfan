...
function acc_ledgerCtrl($scope, $http){	
	$scope.auth=getAuth();
	this.init($scope);	
	
	//Grid,dropdown data loading
	loadGridData($scope.pagingOptions.pageSize,1);
			loadData('get_acc_group_list',{}).success(function(data){$scope.acc_groupList=data;});
	...
	$scope.saveItem=function(){	
		...
		angular.extend(record,$scope.item);
				record.acc_group_Group_Name=undefined;
	...
	$scope.editItem=function(row){	
		...
		$scope.item.create_date=getDate($scope.item.create_date);
	...
	function loadData(action,data){
		return $http({
			  method: 'POST',
			  url: BASE_URL+'acc_ledger_ctrl/'+action,
			  data: data,
 ...
 acc_ledgerCtrl.prototype.init=function($scope){
	$scope.search=null;
	$scope.item=null;
	$scope.list = null;
...
acc_ledgerCtrl.prototype.configureGrid=function($scope){
	$scope.totalItems = 0;
    $scope.pagingOptions = {
    ...
    $scope.gridOptions = { 
        ...
        columnDefs: [
				...
				,{field: 'acc_group_Group_Name', displayName: 'acc_group'}
...
acc_ledgerCtrl.prototype.searchPopup=function($scope){
