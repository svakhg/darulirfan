...
function departmentCtrl($scope, $http){	
	...
	function loadData(action,data){
		return $http({
			  method: 'POST',
			  url: BASE_URL+'department_ctrl/'+action,
			  data: data,
 ...
 departmentCtrl.prototype.init=function($scope){
	$scope.search=null;
	$scope.item=null;
	$scope.list = null;
...
departmentCtrl.prototype.configureGrid=function($scope){
	$scope.totalItems = 0;
    $scope.pagingOptions = {
    ...
    $scope.gridOptions = { 
        ...
        columnDefs: [
				{field:'', displayName:'Action', width:actionWidth,	cellTemplate:'<div style="position:relative;top:4px;padding-left:2px"><button ng-show="auth.update"  ng-click="editItem(row)" class="btn btn-primary btn-mini" ><i class="icon-edit icon-white"></i> Edit</button>&nbsp;<button ng-show="auth.delete" ng-click="deleteItem(row)" class="btn btn-danger btn-mini"><i class="icon-trash icon-white"></i> Delete</button> </div>'
								...
								,{field: 'Department_Name', displayName: 'Department_Name'}
...
departmentCtrl.prototype.searchPopup=function($scope){
