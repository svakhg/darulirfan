...
function designationCtrl($scope, $http){	
	$scope.auth=getAuth();
	this.init($scope);	
	...
	function loadData(action,data){
		return $http({
			  method: 'POST',
			  url: BASE_URL+'designation_ctrl/'+action,
			  data: data,
 ...
 designationCtrl.prototype.init=function($scope){
	$scope.search=null;
	$scope.item=null;
	$scope.list = null;
...
designationCtrl.prototype.configureGrid=function($scope){
	$scope.totalItems = 0;
    $scope.pagingOptions = {
    ...
    $scope.gridOptions = { 
        data: 'list',
        columnDefs: [
				{field:'', displayName:'Action', width:actionWidth,	cellTemplate:'<table class="table"><button ng-show="auth.update"  ng-click="editItem(row)" class="btn btn-primary btn-mini" ><i class="icon-edit icon-white"></i> Edit</button>&nbsp;<button ng-show="auth.delete" ng-click="deleteItem(row)" class="btn btn-danger btn-mini"><i class="icon-trash icon-white"></i> Delete</button> </table>'
				}
								,{field: 'Designation_Name', displayName: 'Designation_Name'}
...
designationCtrl.prototype.searchPopup=function($scope){
